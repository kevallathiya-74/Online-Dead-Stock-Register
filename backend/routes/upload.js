...
router.get(
  ...
  validateObjectId("asset_id"),
  uploadController.getAssetDocuments,
);
...
router.use((error, req, res, next) => {
  if (error.code === "LIMIT_FILE_SIZE") {
    return res.status(400).json({
      message:
        "File too large. Maximum size allowed is 10MB for documents and 5MB for images.",
    });
  }
  ...
  if (error.code === "LIMIT_FILE_COUNT") {
    return res.status(400).json({
      message:
        "Too many files. Maximum 5 documents or 10 images allowed at once.",
    });
  }
