...
class AssetService {
    ...
    try {
      ...
      let query = supabase.from("assets").select(
        ...
        *,
        ...
        vendor:vendors(id, company_name, contact_person, email)
      `,
        { count: "exact" },
      );
      ...
      if (filters.search) {
        query = query.or(
          `unique_asset_id.ilike.%${filters.search}%,manufacturer.ilike.%${filters.search}%,model.ilike.%${filters.search}%,serial_number.ilike.%${filters.search}%`,
        );
      }
  ...
  async getAssetById(assetId) {
    ...
    try {
      ...
      if (auditError) {
        logger.warn("Failed to fetch audit history", {
          error: auditError,
          assetId,
        });
      }
  ...
  async updateAsset(assetId, updateData, userId) {
    ...
    try {
      ...
      if (auditError) {
        logger.warn("Failed to create audit log for asset update", {
          error: auditError,
        });
      }
  ...
  async deleteAsset(assetId, userId) {
    ...
    try {
      ...
      if (auditError) {
        logger.warn("Failed to create audit log for asset deletion", {
          error: auditError,
        });
      }
  ...
  async getUserAssets(userId) {
    ...
    try {
      ...
      logger.error("Error fetching user assets", {
        error: error.message,
        userId,
      });
      throw error;
  ...
  async getAssetStats(user) {
    ...
    try {
      ...
      const totalValue = (assetsWithCost || []).reduce(
      ...
      return {
        ...
        byStatus: Object.entries(statusCounts).map(([_id, count]) => ({
          _id,
          count,
        })),
        byDepartment: Object.entries(byDepartment).map(([_id, count]) => ({
          _id,
          count,
        })),
