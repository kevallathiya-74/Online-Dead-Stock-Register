...
exports.exportData = async (req, res) => {
  try {
    ...
    // Export Assets
    if (includeAssets) {
      const { data: assets, error: assetError } = await supabase.from("assets")
        .select(`
          *,
          assigned_user_info:users!assigned_user(name, email)
        `);
      ...
      exportData.data.assets = (assets || []).map((asset) => ({
        ...
        assigned_user: asset.assigned_user_info?.name || "",
        assigned_user_email: asset.assigned_user_info?.email || "",
    ...
    if (includeUsers) {
      const { data: users, error: userError } = await supabase
        .from("users")
        .select(
          "id, name, email, role, department, employee_id, phone, is_active, last_login, created_at, updated_at",
        );
    ...
    if (includeTransactions) {
      const { data: transactions, error: txnError } = await supabase.from(
        "transactions",
      ).select(`
          *,
          asset_info:assets!asset_id(unique_asset_id, manufacturer, model),
          from_user_info:users!from_user(name, email),
          to_user_info:users!to_user(name, email),
          approved_by_info:users!approved_by(name, email)
        `);
      ...
      exportData.data.transactions = (transactions || []).map((txn) => ({
        ...
        asset_id: txn.asset_info?.unique_asset_id || "",
        asset_info: txn.asset_info
          ? `${txn.asset_info.manufacturer} ${txn.asset_info.model}`
          : "",
        from_user: txn.from_user_info?.name || "",
        from_user_email: txn.from_user_info?.email || "",
        to_user: txn.to_user_info?.name || "",
        to_user_email: txn.to_user_info?.email || "",
        from_location: txn.from_location,
        to_location: txn.to_location,
        quantity: txn.quantity,
        transaction_date: txn.transaction_date,
        status: txn.status,
        approved_by: txn.approved_by_info?.name || "",
        notes: txn.notes,
    ...
    if (format === "json") {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="system-export-${Date.now()}.json"`,
      );
      res.json(exportData);
      ...
      if (
        ...
        exportData.data.transactions.length > 0
      ) {
      ...
      const summaryData = [
        ...
        { Field: "Total Users", Value: exportData.statistics.totalUsers },
        {
          ...
          Value: exportData.statistics.totalTransactions,
        },
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="system-export-${Date.now()}.xlsx"`,
      );
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="system-export-${Date.now()}.csv"`,
      );
...
exports.importData = async (req, res) => {
  try {
    ...
    const format =
      req.body.format ||
      (req.file.originalname.endsWith(".csv") ? "csv" : "json");
      ...
      case "assets": {
        ...
        for (let i = 0; i < importData.length; i++) {
          ...
          if (!asset.unique_asset_id) {
            asset.unique_asset_id = `AST-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
          }
          ...
          validAssets.push({
            ...
            purchase_cost: asset.purchase_cost
              ...
              : 0,
            warranty_expiry: asset.warranty_expiry
              ? new Date(asset.warranty_expiry).toISOString()
              : null,
            condition: asset.condition || "Good",
            assigned_user: null, // Default unassigned for import
          });
        }
