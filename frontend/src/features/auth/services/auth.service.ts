...
export const authService = {
  ...
  async resetPassword(email: string): Promise<{ error: JWTAuthError | null }> {
    try {
      ...
      return {
        error: createAuthError(
          (error as { response?: { data?: { message?: string } } }).response?.data?.message || 'Password reset request failed',
          (error as any).response?.status || 500
        )
      };
    }
  ...
  async updatePassword(newPassword: string, token?: string): Promise<{ error: JWTAuthError | null }> {
    try {
      ...
      return {
        error: createAuthError(
          (error as { response?: { data?: { message?: string } } }).response?.data?.message || 'Password update failed',
          (error as any).response?.status || 500
        )
      };
    }
