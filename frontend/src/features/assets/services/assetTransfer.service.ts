...
class AssetTransferService {
  ...
  async createTransferRequest(transferRequest: AssetTransferRequest): Promise<AssetTransfer> {
    try {
      ...
      throw new Error((error as { response?: { data?: { message?: string } } }).response?.data?.message || (error as Error).message || 'Failed to create asset transfer request');
    }
  }
  ...
  async getUserTransfers(): Promise<AssetTransfer[]> {
    try {
      ...
      throw new Error((error as { response?: { data?: { message?: string } } }).response?.data?.message || (error as Error).message || 'Failed to fetch asset transfers');
    }
  }
  ...
  async getTransferById(transferId: string): Promise<AssetTransfer> {
    try {
      ...
      throw new Error((error as { response?: { data?: { message?: string } } }).response?.data?.message || (error as Error).message || 'Failed to fetch asset transfer details');
    }
  }
}
